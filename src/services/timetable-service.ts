...
export class TimetableService {
  ...
  public validateLesson(lesson: Lesson): ScheduleConflict | null {
    const lessonsInSameTime = this.schedule.filter(
      (lsn) => lsn.timeSlot == lesson.timeSlot && lsn.dayOfWeek == lesson.dayOfWeek
    );
  ...
  public getClassroomUtilization(classroom: Classroom): number {
    const numberOfLessons = this.schedule.filter(
      (lesson) => lesson.classroomNumber == classroom.number
    ).length;
  ...
  public getSchedule(): ReadonlyArray<Lesson> {
    return this.schedule;
  }
  public getClassrooms(): ReadonlyArray<Classroom> {
    return this.classrooms;
  }
  public getCourses(): ReadonlyArray<Course> {
    return this.courses;
  }
  public getProfessors(): ReadonlyArray<Professor> {
